...
public:
    ...
    ChatCommand* GetCommands() const override
    {
        static ChatCommand commandTable[] =
        {
            ...
            { "kill",             rbac::RBAC_PERM_COMMAND_DIE,              false, &HandleDieCommand,              "", NULL },
            ...
            { "playerinfo",       rbac::RBAC_PERM_COMMAND_PINFO,             true, &HandlePInfoCommand,            "", NULL },
            ...
            { "return",           rbac::RBAC_PERM_COMMAND_RECALL,           false, &HandleRecallCommand,           "", NULL },
			...
			{ "haircut",          rbac::RBAC_PERM_COMMAND_HAIRCUT,          false, &HandleHaircutCommand,          "", NULL },
    ...
    static bool HandleUnAuraCommand(ChatHandler* handler, char const* args)
    {
		...
		else
		{
			...
			{
				handler->SendSysMessage("Removing auras from NPCs is prohibited in this zone.");
				return true;
			}
		}
    ...
    static bool HandleAddItemCommand(ChatHandler* handler, char const* args)
    {
		...
		if (!handler->GetSession()->GetPlayer()->CanUseID(DISABLE_TYPE_ITEM, itemId) && !handler->GetSession()->GetPlayer()->IsAdmin())
		{
			handler->PSendSysMessage("This item (id '%u') is disabled.", itemId);
			return true;
		}

		if (!playerTarget->GetCommandStatus(TOGGLE_MODIFY) && !handler->GetSession()->GetPlayer()->IsAdmin() && handler->GetSession()->GetPlayer()->GetGUID() != playerTarget->GetGUID())
		{
    ...
    */
	...
	static bool HandleHaircutCommand(ChatHandler* handler, char const* /*args*/)
	{
		Player* player = handler->GetSession()->GetPlayer();

		handler->GetSession()->SendShowBarberShop(player->GetGUID());
		return true;
	}
};
